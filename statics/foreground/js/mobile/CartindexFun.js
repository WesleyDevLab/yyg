    ...
    var b = function() {
        ...
        if (h.length > 0) {
            h.children("a").click(function() {
                location.href = Gobal.Webpath+"/cart/pay"   //付款页面
            })
        }
        ...
        var m = function() {
            ...
            $("input:text[name=num]", a).each(function(s) {
                ...
                if (!isNaN(t)) {
                    var price = parseInt( $(this).attr('data-money') );			//roby 修改单价不为1时的情况
                    if (!isNaN(t)) {
                    	r++;
                        q += t*price; 
                    }                    
            ...
            if (r > 0) {
                ...
                g.children("p").html('总共购买<span class="orange arial z-user">' + r + '</span>个商品  合计元宝：<span class="orange arial">' + q + "</span> ")
            } else {
                g.remove()
            }
        };
        ...
        var j = function(q, t, r) {
            ...
            var s = function(w) {
                if (w.code == 1) {
                    ...
                    if (w.code == 0) {
                    	var price = parseInt(q.attr('data-money'));
                    	var money = r*price;
                        q.parent().prev().html('总共购买：<em class="arial">' + r + '</em>人次/<em class="orange arial">￥' + money + ".00</em>")
                    }
                }
            };
        ...
        $("a[name=delLink]", a).each(function(q) {
            ...
            function() {			 
                ...
                var s = function() {
                    var u = function(v) {					 
                        if (v.code == 0) {
                            GetJPData(Gobal.Webpath, "ajax", "cartnum", function(data){
                                //console.log(data);
