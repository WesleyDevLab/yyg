...
<title>列表页-<?php echo $title;?></title>
...
<script src="<?php echo ASSET_FONT;?>/js/mobile/goods.js?v=25" language="javascript" type="text/javascript"></script>
...
<header class="header">
    <div class="n-header-wrap">
        ...
        <div class="n-h-tit"><h1 class="header-logo"><?php echo $title;?></h1></div>
    </div>
</header>
	<?php
	if($type=='soon'){
	...
	<section class="revealed">
		<div id="divLottery" class="revCon">
			<div id="divLotteryLoading" class="loading" style="display: none;"><b></b>正在加载</div>
		    <a id="btnLoadMore" class="loading" href="javascript:;" style="display: none;">点击加载更多</a>
		    <a id="btnLoadMore2" class="loading" style="display:none;">没有数据</a>
		    <a id="btnLoadMore3" class="loading" style="display: block;">已经到底了</a>
		</div>
    </section>	
	<?php	
	}
...
function glist_json(parm){
	$("#urladdress").val('');
	$("#pagenum").val('');
	$.getJSON('<?php echo G_WEB_PATH?>/goods/jiejiaoajax/'+parm,function(data){
		console.log(data);
		...
		if(data[0].sum){
			...
			if($type=='soon'){
				...
				$htmlid = 'divLotteryLoading';
			}
			?>
			handleData(data,'<?php echo G_WEB_PATH;?>','<?php echo $htmlid;?>');				
...
$(document).ready(function(){
	...
	//跳转页面
	var gt='.goodsList span, .goodsList h2, .goodsList, .Progress-bar, .revConL';
