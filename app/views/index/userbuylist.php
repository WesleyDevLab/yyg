...
<header class="header">
    <div class="n-header-wrap">
        ...
        <div class="n-h-tit"><h1 class="header-logo">所有参与记录</h1></div>
    </div>
</header>
...
function glist_json(parm){
	...
	$("#pagenum").val('');
	console.log(parm);
	$.getJSON('<?php echo G_WEB_PATH?>/goods/getGoodBuyList/'+parm,function(data){
		console.log(data);
		...
		if(data[0].sum){
			...
			var ul = '<div class="participation_list">';	
	        for(var i=0;i<data.length;i++){
	           var timestamp = new Date(parseInt(data[i].btime)*1000);	
			   var uphoto = data[i].uphoto;
			   if(uphoto.indexOf("http") < 0 )	{
			   	uphoto = "http://"+uphoto;
			   }	           
			   ul += '<ul>'+
			            '<li>'+
			                '<div class="participation_detail">'+
			                    '<img src="'+data[i].uphoto+'">'+
			                    '<div class="takename_date">'+
			                        '<p class="join_name">'+data[i].username+'</p>'+
			                        '<p class="purchase_number" style="font-weight: bold;">购买号码:'+data[i].code_duan+'</p>'+
			                        '<p>'+data[i].ip+'</p>'+
			                        '<p>' + dateToStr(timestamp) +
			                    '</div>'+
			                '</div>'+
			                '<div class="ren_number">'+
			                    	'参与<ins style="color:#F5484C;">'+data[i].gonumber+'</ins>人次'+
			                '</div>'+
			            '</li>'+
		              '</ul>';	
			}
